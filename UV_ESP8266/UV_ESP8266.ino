...
WiFiUDP ntpUDP;
NTPClient timeClient(ntpUDP, "ntp.aliyun.com", 28800, 60000); // UTC+8 for China, 使用阿里云的NTP服务器
...
void setup() {
  ...
  // 初始化NTP
  timeClient.begin();
  timeClient.setTimeOffset(28800); // 确保设置了正确的时区偏移
  if(!timeClient.update()) {
    Serial.println("NTP同步失败!");
  } else {
    Serial.println("NTP同步成功!");
    currentDate = String(timeClient.getEpochTime() / 86400L * 86400L);
  }
...
void handleRoot() {
  ...
  html += "function updateHistoricalData() {";
  ...
  html += "  const dateStr = `${year}-${month}-${day}`;";
  html += "  document.getElementById('date').value = dateStr;";
  html += "  updateTime();";
  html += "  updateUVData();";
  html += "  updateHistoricalData();";
  
  html += "  setInterval(updateTime, 1000);";
  html += "  setInterval(updateUVData, 1000);";
  html += "  setInterval(updateHistoricalData, 5000);";
  html += "};";
  ...
  html += "  let html = '<table><tr><th>时间</th><th>UV指数</th></tr>';";
...
void saveUVData() {
  ...
  String filename = "/" + String(ptm->tm_year + 1900) + month + day + ".txt";
  
  if(!LittleFS.exists("/")) {
    Serial.println("文件系统未就绪，尝试重新初始化...");
    initializeLittleFS();
  }
  
  File file = LittleFS.open(filename, "a");
  if(!file) {
    Serial.println("打开文件失败: " + filename);
    ...
    if(LittleFS.info(fs_info)) {
      ...
      if(fs_info.totalBytes - fs_info.usedBytes < 1024) {
        Serial.println("存储空间不足，尝试清理旧文件...");
      }
    }
    return;
  }
  
  String dataLine = String(timeClient.getEpochTime()) + "," + String(uv);
...
void handleData() {
  if(server.hasArg("date")) {
    ...
    if(LittleFS.exists(filename)) {
      ...
      server.send(404, "text/plain", "当前日期无数据记录");
    }
  } else {
...
String getFormattedDateTime() {
  time_t epochTime = timeClient.getEpochTime();
  struct tm *ptm = gmtime ((time_t *)&epochTime);
  String dateTime = String(ptm->tm_year + 1900) + "年";
  dateTime += String(ptm->tm_mon + 1) + "月";
  dateTime += String(ptm->tm_mday) + "日 ";
